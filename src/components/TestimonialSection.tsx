...
/* ── Directional horizontal slide variants ───────────────────────────────
 * Mirrors the x-axis physics of the "Latest Work" / "Events" tracks.
 *   direction = 1  (Right arrow → next): old exits left (-100%),
 *                                        new enters from the right (+100%).
 *   direction = -1 (Left arrow → prev):  old exits right (+100%),
 *                                        new enters from the left (-100%).
 */
const slideVariants = {
...
export default function TestimonialSection({
  ...
  // Desktop paged slider — track page AND the direction of travel so the
  // exit/enter sides flip to follow the clicked arrow.
  ...
  // Advance the page and remember which way we travelled so the slide
  // variants resolve to the correct entry/exit sides.
  const paginate = (newDirection: number) => {
    setDirection(newDirection);
    setPage((prev) =>
      Math.min(maxPage, Math.max(0, prev + newDirection)),
    );
  };
        ...
        {ready && (
          <>
            ...
            <div className="relative hidden md:block group/slider">
              {/* overflow-hidden clips the full-width horizontal slide */}
              ...
              {shuffledTestimonials.length > perPage && (
                <>
                  <button
                    className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 opacity-0 group-hover/slider:opacity-100 transition-opacity duration-300 w-12 h-12 flex items-center justify-center bg-surface/90 shadow-card border border-outline-variant/10 z-10"
                    onClick={() => paginate(-1)}
                  ...
                  <button
                    className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 opacity-0 group-hover/slider:opacity-100 transition-opacity duration-300 w-12 h-12 flex items-center justify-center bg-surface/90 shadow-card border border-outline-variant/10 z-10"
                    onClick={() => paginate(1)}
